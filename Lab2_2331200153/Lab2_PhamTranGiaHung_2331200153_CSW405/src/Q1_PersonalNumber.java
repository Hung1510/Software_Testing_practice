...
public class Q1_PersonalNumber {
     ...
     * 10-digit personal number string (YYMMDDXYZC)
     * @throws IllegalArgumentException if personal number is invalid
     */
    public Q1_PersonalNumber(String personalNumber) {
        // Validate format
        if (personalNumber == null || personalNumber.length() != 10) {
            throw new IllegalArgumentException("Personal number only 10 digits");
        }
        
        // Check characters are digits
        if (!personalNumber.matches("\\d{10}")) {
            throw new IllegalArgumentException("Personal number contain only digits");
        }
