...
public class ParkingGarageTest {
    ...
    private Car car1;
    private Car car2;
    private LowEmissionCar eco1;
    private CargoTruck truck1;
    private CargoTruck truck2;
    ...
    public void setUp() {
        ...
        car1 = new Car("ABC123", 5);
        car2 = new Car("XYZ789", 4);
        eco1 = new LowEmissionCar("ECO001", 4);
        truck1 = new CargoTruck("TRK100", 25000); // 3 spaces
        truck2 = new CargoTruck("TRK200", 50000); // 5 spaces
    }
    ...
    public void testInsufficientSpace() throws Exception {
        ...
        garage.enter(new Car("CAR3", 5)); // 1 space
        garage.enter(new Car("CAR4", 5)); // 1 space
        garage.enter(new Car("CAR5", 5)); // 1 space
        garage.enter(new Car("CAR6", 5)); // 1 space
        garage.enter(new Car("CAR7", 5)); // 1 space
        garage.enter(new Car("CAR8", 5)); // 1 space
    ...
    public void testGetVehicle() throws Exception {
        ...
        Vehicle retrieved = garage.getVehicle("ABC123");
        assertNotNull(retrieved);
        assertEquals(car1, retrieved);
        assertEquals("ABC123", retrieved.getLicenseNumber());
        
        Vehicle notFound = garage.getVehicle("NOTHERE");
        assertNull(notFound);
    }
    ...
    public void testVehicleCreation() {
        ...
        // Test LowEmissionCar
        LowEmissionCar eco = new LowEmissionCar("TEST2", 4);
        ...
        // Test CargoTruck
        CargoTruck truck = new CargoTruck("TEST3", 30000);
    ...
    public void testInvalidCarCapacity() {
        new Car("TEST", 0); // Should throw exception
    }
    
    @Test(expected = IllegalArgumentException.class)
    public void testInvalidTruckWeight() {
        new CargoTruck("TEST", -1000); // Should throw exception
    }
    
    @Test(expected = IllegalArgumentException.class)
    public void testInvalidGarageCapacity() {
        new ParkingGarage(0); // Should throw exception
    }
    
    @Test
    public void testComplexScenario() throws Exception {
        // Simulate a busy day at the garage
        ParkingGarage busyGarage = new ParkingGarage(20);
        
        // Morning rush - cars arriving
        busyGarage.enter(new Car("CAR001", 5));
        busyGarage.enter(new Car("CAR002", 4));
        busyGarage.enter(new LowEmissionCar("ECO001", 2));
        busyGarage.enter(new LowEmissionCar("ECO002", 4));
        ...
        busyGarage.enter(new CargoTruck("TRK001", 20000)); // 2 spaces, $20
        busyGarage.enter(new CargoTruck("TRK002", 35000)); // 4 spaces, $35
        ...
        busyGarage.enter(new Car("CAR003", 5));
        busyGarage.enter(new Car("CAR004", 3));
